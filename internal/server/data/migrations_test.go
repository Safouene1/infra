...
`
		...
		{
			label: testCaseLine("2022-11-01T11:11"),
			setup: func(t *testing.T, tx WriteTxn) {
			...
			expected: func(t *testing.T, tx WriteTxn) {
				// check the relation of group to provider
				var count int
				...
				err = tx.QueryRow(`SELECT COUNT(*) FROM identities_groups WHERE provider_id=12345`).Scan(&count)
				assert.NilError(t, err)
				assert.Equal(t, count, 2) // Developers, Everyone
				err = tx.QueryRow(`SELECT COUNT(*) FROM identities_groups WHERE provider_id=54321`).Scan(&count)
				assert.NilError(t, err)
				assert.Equal(t, count, 1) // Developers
				err = tx.QueryRow(`SELECT COUNT(*) FROM identities_groups WHERE provider_id=12346`).Scan(&count)
				assert.NilError(t, err)
				assert.Equal(t, count, 2) // Developers, Ops
				err = tx.QueryRow(`SELECT COUNT(*) FROM identities_groups WHERE provider_id=1001`).Scan(&count)
				assert.NilError(t, err)
				assert.Equal(t, count, 1) // Local
			},
		},
	}
